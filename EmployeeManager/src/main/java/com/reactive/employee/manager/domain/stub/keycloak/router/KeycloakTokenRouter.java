...
@Configuration
public class KeycloakTokenRouter {

    @Bean
    RouterFunction<ServerResponse> routeToken(KeycloakHandler handler) {
        return route()
                .path("/realm_hr_stub/protocol/openid-connect/token",
                        builder -> builder
                                .nest(accept(APPLICATION_JSON).or(contentType(APPLICATION_JSON)).or(accept(TEXT_EVENT_STREAM)),
                                        nestedBuilder -> nestedBuilder
                                                .POST( handler::requestForToken)
                                )
                ).build();
    }
}
