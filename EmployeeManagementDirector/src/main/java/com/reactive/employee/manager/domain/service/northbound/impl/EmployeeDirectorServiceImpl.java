...
@Service
public class EmployeeDirectorServiceImpl implements EmployeeDirectorService {
    ...
    public static final Logger Employee_Director_Logger = LoggerFactory.getLogger(EmployeeDirectorServiceImpl.class);
    ...
    @Override
    public Employee getEmployeeFromManager(String username) throws EmployeeDirectorException {

        KeycloakUser keycloakUser = employeeManagerService.getSystemUser(username);
        keycloakUser.setClient_id("getEmployee");
        Employee employee = employeeManagerService.getEmployee(username, keycloakUser);
        return employee;
    }
    ...
    public Mono<ServerResponse> validateVacation(Employee employee, Vacation vacationRequest) throws EmployeeDirectorException{
        ...
        if(diff > 360){
            Employee_Director_Logger.info("Employee {} has right to renew vacation days", employee.getUsername());
            ...
            employeeManagerService.patchEmployee(employee.getUsername(),employee,keycloakUser);
            Employee_Director_Logger.info("Employee {} remaining vacation days is renewed to {}", employee.getUsername(), Float.toString(days));
        }
    ...
    //TODO APP Vacation To Manager's approval List
    public Mono<ServerResponse> addVacationToApprovalList(List<Employee> reportsToEmployee, Vacation vacationRequest, Employee employee) throws EmployeeDirectorException{
        ...
        for(Employee manager : reportsToEmployee){
            ...
            employeeManagerService.patchEmployee(manager.getUsername(),patchEmployee,keycloakUser);
            Employee_Director_Logger.info("Vacation Request is added to approval list");
        }
        employee.setVacations(List.of(vacationRequest));
        //TODO add vacation to vacationlist of Employee
        Employee_Director_Logger.info("Vacation Request is added to employee vacation list");
        employeeManagerService.patchEmployee(employee.getUsername(),employee,keycloakUser);
    ...
    public Mono<ServerResponse> approveVacationRequest(Employee employee_approver, Vacation vacationRequest) throws EmployeeDirectorException {
        ...
        employeeManagerService.patchEmployee(requesterEmployee.getUsername(),employee,keycloakUser);
        employeeManagerService.patchEmployee(employee_approver.getUsername(),manager, keycloakUser);
        Employee_Director_Logger.info("Vacation Request is approved for user : {}", requesterEmployee.getUsername());
        return ServerResponse.status(HttpStatus.OK)
                .contentType(APPLICATION_JSON)
                .body(Mono.just(vacationRequest), EmployeeDirectorException.class);
    }
    ...
    @Override
    public Mono<ServerResponse> patchEmployee(String system_username, Employee employeePatched) {
        KeycloakUser keycloakUser = employeeManagerService.getSystemUser(system_username);
        keycloakUser.setClient_id("manageEmployee");
        Employee employee = employeeManagerService.patchEmployee(system_username,employeePatched, keycloakUser);
        return ServerResponse.status(HttpStatus.OK)
                .contentType(APPLICATION_JSON)
                .body(Mono.just(employee), EmployeeDirectorException.class);
    }
    ...
    public Mono<ServerResponse> createEmployee(String system_username, Employee employee) throws EmployeeDirectorException {
        KeycloakUser keycloakUser = employeeManagerService.getSystemUser(system_username);
        keycloakUser.setClient_id("manageEmployee");
        Employee employeeCreated = employeeManagerService.createEmployee(employee, keycloakUser);
        return ServerResponse.status(HttpStatus.OK)
                .contentType(APPLICATION_JSON)
                .body(Mono.just(employeeCreated), EmployeeDirectorException.class);
    }
    ...
    public Mono<ServerResponse> deleteEmployee(String system_username, String employeeUserName) throws EmployeeDirectorException {
        KeycloakUser keycloakUser = employeeManagerService.getSystemUser(system_username);
        keycloakUser.setClient_id("manageEmployee");
        employeeManagerService.deleteEmployee(employeeUserName, keycloakUser);
        return ServerResponse.status(HttpStatus.OK)
                .contentType(APPLICATION_JSON)
    ...
    @Override
    public Mono<ServerResponse> getAllEmployeesByCriteria(Employee employee, String system_username) {

        KeycloakUser keycloakUser = employeeManagerService.getSystemUser(system_username);
        keycloakUser.setClient_id("manageEmployee");
        List<Employee> employeeList = employeeManagerService.getEmployeeByCriteria(employee,keycloakUser);
        return ServerResponse.status(HttpStatus.OK)
                .contentType(APPLICATION_JSON)
                .body(fromValue(employeeList));
